            ...
            new PostBackFlow(){
                ...
                public Optional<List<Message>> postHandle(Source source) {
                    ...
                    if(InputMapping.getInstance(user.getClassRoom()).get().isOauthRequired()){
                        FlexContainer container = FlexMessages.LoadContainerFromJson(FlexMessages.LoadJsonFromJsonFile("oauth-required").replace("%uri",Forms.getEditableFormUri(user,data).toString()));
